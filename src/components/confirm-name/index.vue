<template>
  <transition name="confirm">
    <div class="confirm-name" v-if="isShow" :style="{ 'z-index': zIndex }">
      <div class="confirm-title main-color">{{ $i18n("confirm.txt1", "付款人开户名") }}</div>
      ...
      <div class="confirm-btns flex">
        <div class="confirm-btn cancel-btn" @click="close">{{ $i18n("confirm.txt6", "取消") }}</div>
        <div class="confirm-btn common-btn" @click="confirm">{{ $i18n("confirm.txt5", "确定") }}</div>
      </div>
    </div>
  </transition>
</template>
