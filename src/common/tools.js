...
tools.openNewWindow = function (url, isFullScreen, vurRef) {
  ...
  function open() {
    return window.open(url, "_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width=" + w + ",height=" + h + ",left=" + left + ",top=" + top)
  }
  let ref = open()
